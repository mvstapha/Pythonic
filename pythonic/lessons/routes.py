...
def new_lesson():
    ...
    if form == "new_lesson_form" and new_lesson_form.validate_on_submit():
        ...
        if new_course_form.icon.data:
            picture_file = save_picture(new_course_form.icon.data, 'static/course_icons', output_size=(150,150))
        course_title = str(new_course_form.title.data)
        course = Course(
            title=course_title,
...
def lesson(course, lesson_slug):
    ...
    return render_template('lesson_view.html', title=lesson.title, lesson=lesson, previous_lesson=previous_lesson, next_lesson=next_lesson)
...
def update_lesson(course, lesson_slug):
    ...
    return render_template('update_lesson.html', title='Update | ' +lesson.title, lesson=lesson, previous_lesson=previous_lesson, next_lesson=next_lesson, form=form)
